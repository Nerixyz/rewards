use std::env;
use dialoguer::{Input, Password, console::style, Confirm};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // language=PostgreSQL
    if pg.query_opt("SELECT key FROM config WHERE key = 'user_token'", &[]).await?.is_some() {
        // language=none
    ...
    let tw_client_id = ClientId::new(client_id.clone());
    let tw_client_secret = ClientSecret::new(client_secret);

    println!("SeemsGood. Make sure you have {} added as a redirect-url in the dev-console!\n", style("http://localhost").cyan());
    println!("Next, authenticate the app and paste either the {} or the {} here.", style("code").green(), style("full url").red());

    let mut builder = UserTokenBuilder::new(
        tw_client_id,
        tw_client_secret,
        RedirectUrl::new("http://localhost".to_string())?)?.set_scopes(vec![Scope::ChatEdit,
                            Scope::ChatRead,
    ...
    let input = Input::<String>::new().with_prompt("Code or Url").interact()?;
    ...
    let token = builder.get_user_token(reqwest_http_client, csrf.secret(), &code).await?;
