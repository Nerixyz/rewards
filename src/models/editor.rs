...
impl Editor {
    ...
    pub async fn get_broadcaster_for_editor(editor_id: &str, broadcaster_id: &str, pool: &PgPool) -> Result<User, SqlError> {
        // TODO: SELECT * ?!?!?!?
    ...
    pub async fn add_editor(broadcaster_id: &str, editor_name: &str, pool: &PgPool) -> Result<(), SqlError> {
        let _ = sqlx::query!(
            // language=PostgreSQL
            r#"
            INSERT INTO editors
            ...
            "#, editor_name, broadcaster_id)
            .execute(pool)
            .await?;
        Ok(())
    }

    pub async fn delete_editor(broadcaster_id: &str, editor_name: &str, pool: &PgPool) -> Result<(), SqlError> {
