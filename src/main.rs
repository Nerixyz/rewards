...
use crate::services::eventsub::{register_eventsub_for_all_unregistered, clear_invalid_rewards};
...
async fn main() -> std::io::Result<()> {
    ...
    let app_access_token = get_app_access_token().await.expect("Could not get app access token");
    let app_access_token = web::Data::new(Mutex::new(app_access_token));
    let _refresh_actor = TokenRefresher::new(pool.clone()).start();

    clear_invalid_rewards(&app_access_token, &pool).await.expect("Could not clear invalid rewards");
    register_eventsub_for_all_unregistered(&app_access_token, &pool).await.expect("Could not register eventsub FeelsMan");
