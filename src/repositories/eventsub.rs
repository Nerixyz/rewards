use actix_web::{HttpResponse, Error, web::{self, ServiceConfig}, post};
use sqlx::PgPool;
    ...
    match payload.into_inner() {
        ...
        Payload::ChannelPointsCustomRewardRedemptionAddV1(redemption) => {
            ...
            actix_web::rt::spawn(async move {
                ...
                if let (Ok(reward), Ok(user_token)) = (reward, User::get_by_id(&broadcaster_id, &pool).await) {
