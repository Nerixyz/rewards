...
#[put("/{broadcaster_id}")]
async fn create(claims: JwtClaims, pool: web::Data<PgPool>, body: web::Json<CreateRewardBody>, broadcaster_id: web::Path<String>) -> Result<HttpResponse, Error> {
...
async fn update(claims: JwtClaims, pool: web::Data<PgPool>, body: web::Json<UpdateRewardBody>, path: web::Path<(String, String)>) -> Result<HttpResponse, Error> {
...
async fn delete(claims: JwtClaims, pool: web::Data<PgPool>, path: web::Path<(String, String)>) -> Result<HttpResponse, Error> {
    let (broadcaster_id, reward_id) = path.into_inner();
    let token = get_user_or_editor(&claims, &broadcaster_id, &pool).await?.into();
...
#[get("/{broadcaster_id}")]
async fn list_for_user(claims: JwtClaims, pool: web::Data<PgPool>, broadcaster_id: web::Path<String>) -> Result<HttpResponse, Error> {
    let token = get_user_or_editor(&claims, &broadcaster_id, &pool).await?.into();

    let (rewards, saved_rewards) = futures::future::join(get_rewards_for_id(&broadcaster_id, &token), Reward::get_all_for_user(&broadcaster_id, &pool)).await;
...
async fn get_reward(claims: JwtClaims, pool: web::Data<PgPool>, path: web::Path<(String, String)>) -> Result<HttpResponse, Error> {
    let (broadcaster_id, reward_id) = path.into_inner();
    let token = get_user_or_editor(&claims, &broadcaster_id, &pool).await?.into();
