...
use crate::services::bttv::requests::{
    get_dashboards, get_user_by_twitch_id, BttvLimits,
};
use models::user::User;

pub mod requests;

pub async fn fetch_save_bttv_id(
    user_id: &str,
    pool: &PgPool,
) -> AnyResult<String> {
...
pub async fn get_or_fetch_id(
    ...
    Ok(if let Some(id) = this_user.bttv_id {
        ...
        fetch_save_bttv_id(user_id, pool).await.map_err(|_| {
            AnyError::msg("The user hasn't registered on bttv yet")
        })?
    })
}
...
pub async fn verify_user(
    ...
    get_user_limits(&bttv_id).await.map_err(|_| {
        AnyError::msg("RewardMore isn't an editor for the user")
    })?;
