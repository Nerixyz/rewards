...
use crate::services::bttv::requests::{get_user_by_twitch_id, BttvLimits, get_dashboards, get_user, get_emote, BttvEmote, delete_shared_emote, add_shared_emote};
...
pub async fn swap_or_add_emote(user_id: &str, emote_id: &str, pool: &PgPool) -> AnyResult<(Option<String>, String)> {
    let this_user = User::get_bttv_data(user_id, pool).await.map_err(|_| AnyError::msg("Internal Error."))?;
    let bttv_id = if let Some(id) = &this_user.bttv_id {
        id.clone()
    } else {
        fetch_save_bttv_id(user_id, pool).await.map_err(|_| AnyError::msg("No such user."))?
    };
    ...
    if bttv_user.shared_emotes.iter().any(|e| e.id == emote_id || e.code == emote_data.code) {
        return Err(AnyError::msg("The emote already exists as a shared emote"))
    }
    if bttv_user.channel_emotes.iter().any(|e| e.id == emote_id || e.code == emote_data.code) {
    ...
    let (removed_emote, mut history) = if bttv_user.shared_emotes.len() >= user_limits.shared_emotes {
        remove_emote(this_user.bttv_history.0, &bttv_id, bttv_user.shared_emotes)
            .await.map_err(|_| AnyError::msg("No emote to be removed."))?
    } else {
    ...
    if let Err(e) = add_shared_emote(emote_id, &bttv_id).await {
        if let Err(sql_err) = User::set_bttv_history(user_id, history, pool).await {
            log::warn!("Error setting bttv history after failing to insert shared emote: sql_error={}", sql_err);
        }
        log::warn!("Could not add shared emote: {}", e);
        return Err(AnyError::msg("Couldn't add shared emote."))
    }
    history.push(emote_id.to_string());
    User::set_bttv_history(user_id, history, pool).await.map_err(|_| AnyError::msg("Internal error"))?;
...
async fn remove_emote(mut history: Vec<String>, bttv_id: &str, shared_emotes: Vec<BttvEmote>) -> AnyResult<(Option<String>, Vec<String>)> {
